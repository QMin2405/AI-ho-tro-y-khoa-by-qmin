...
const MemoizedChatMessage = React.memo(({ msg }: { msg: ChatMessage }) => {
    ...
    return (
        <div className={`flex items-start gap-2.5 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
            ...
            <div className={`p-3 rounded-xl max-w-xs ${msg.sender === 'user' ? 'bg-brand-primary text-white' : 'bg-slate-100 dark:bg-gray-700'}`}>
               <div className="prose prose-sm dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: messageHtml }}></div>
            </div>
        </div>
    );
});
